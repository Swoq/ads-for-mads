package com.swoqe.adsformads.specification.createUserRequest;

import com.swoqe.adsformads.security.dto.CreateUserRequest;
import com.swoqe.adsformads.specification.AbstractSelector;

import java.util.List;
import java.util.regex.Pattern;

public class CredentialsRegularExpressionSelector extends AbstractSelector<CreateUserRequest> {

    private final Pattern passwordPattern = Pattern.compile("^(?=.*[A-Za-z])(?=.*\\d)(?=.*[@$!%*#?&])[A-Za-z\\d@$!%*#?&]{8,}$");
    private final Pattern firstNamePattern = Pattern.compile("\\b([A-ZÀ-ÿ][-,a-z. ']+[ ]*)+");
    private final Pattern lastNamePattern = Pattern.compile("\\b([A-ZÀ-ÿ][-,a-z. ']+[ ]*)+");
    private final Pattern emailPattern = Pattern.compile("(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*|\"(?:[\\x01-\\x08\\x0b\\x0c\\x0e-\\x1f\\x21\\x23-\\x5b\\x5d-\\x7f]|\\\\[\\x01-\\x09\\x0b\\x0c\\x0e-\\x7f])*\")@(?:(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\\[(?:(?:(2(5[0-5]|[0-4][0-9])|1[0-9][0-9]|[1-9]?[0-9]))\\.){3}(?:(2(5[0-5]|[0-4][0-9])|1[0-9][0-9]|[1-9]?[0-9])|[a-z0-9-]*[a-z0-9]:(?:[\\x01-\\x08\\x0b\\x0c\\x0e-\\x1f\\x21-\\x5a\\x53-\\x7f]|\\\\[\\x01-\\x09\\x0b\\x0c\\x0e-\\x7f])+)\\])");


    @Override
    public boolean test(CreateUserRequest createUserRequest) {
        boolean pwd = passwordPattern.matcher(createUserRequest.getPassword()).matches();
        boolean firstName = firstNamePattern.matcher(createUserRequest.getFirstName()).matches();
        boolean lastName = lastNamePattern.matcher(createUserRequest.getLastName()).matches();
        boolean email = emailPattern.matcher(createUserRequest.getEmail()).matches();
        System.out.println(List.of(pwd, firstName, lastName, email));
        return pwd && firstName && lastName && email;

    }
}
